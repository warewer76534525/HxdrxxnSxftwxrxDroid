...
public class LoadingChartData extends InvokeHttpGetConnection {
	private String url;
	...
	public void onReceivedResponse(InputStream is, int length) {
		try {
	        ...
	        if (DataProcessor.getDataContent(data, "status").equals("0")) {
				...
				List<BasicNameValuePair> list = DataProcessor.getChartData(data);
				ChartDataManager.GetInstance().setCurrentData(url, list);
